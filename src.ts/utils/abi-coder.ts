...
export function encodeSignature(fragment: string): string {
    let frag = parseSignature(fragment);
    let sig = formatSignature(frag);
    let hash = keccak_256.update(sig).digest();
    if (frag.type === 'event') {
        return hexlify(hash);
    } else {
