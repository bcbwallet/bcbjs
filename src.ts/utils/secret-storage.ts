...
export function encryptSecret(plain: any, password: Arrayish | string): Uint8Array {
    password = getPassword(password);
    var plainBytes;
    // if (typeof(plain) === 'string') {
    //     let bytes = new Uint8Array(plain.length);
    //     for (let i = 0; i < plain.length; i++) {
    //         bytes[i] = plain.charCodeAt(i);
    //     }
    //     plainBytes = bytes;
    // } else {
        plainBytes = arrayify(plain);
    // }
    var pass = pbkdf2(password, 'e91e2fb05854a2a0a093be59b45944df73e3d0dbfd46e90935f527e324669330', 1, 32, 'sha256');
    var key = pass.slice(0, 16);
    var iv = pass.slice(16);

    // Encrypt
    plainBytes = aes.padding.pkcs7.pad(plainBytes);
    var aesCbc = new aes.ModeOfOperation.cbc(key, iv);
    var cipher = aesCbc.encrypt(plainBytes);

    return cipher;
}

export function decryptSecret(cipher: any, password: Arrayish | string): Uint8Array {
    password = getPassword(password);
    // var cipherBytes = arrayify(cipher);
    ...
    // Decrypt
    var aesCbc = new aes.ModeOfOperation.cbc(key, iv);
    var plain = aesCbc.decrypt(cipher);
    try {
        plain = aes.padding.pkcs7.strip(plain);
    } catch (e) {
...
export function decrypt(json: string, password: Arrayish, progressCallback?: ProgressCallback): Promise<SigningKey> {
    ...
    var getSigningKey = function(key: Uint8Array, reject: (error?: Error) => void) {
        ...
        var chainId;
        if (data.address.startsWith("bcbtest")) {
            chainId = "bcbtest";
        } else if (data.address.startsWith("bcb")) {
            chainId = "bcb";
        } else if (data.address.startsWith("devtest")) {
            chainId = "devtest";
        } else {
            reject(new Error('invalid address'));
            return null;
        }
        var signingKey = new SigningKey(privateKey, chainId);
        // if (signingKey.address !== getAddress(data.address)) {
        //     reject(new Error('address mismatch'));
        //     return null;
        // }

        // Version 0.1 x-bcbjs metadata must contain an encrypted mnemonic phrase
        if (searchPath(data, 'x-bcbjs/version') === '0.1') {
            var mnemonicCiphertext = looseArrayify(searchPath(data, 'x-bcbjs/mnemonicCiphertext'));
            var mnemonicIv = looseArrayify(searchPath(data, 'x-bcbjs/mnemonicCounter'));

            var mnemonicCounter = new aes.Counter(mnemonicIv);
            var mnemonicAesCtr = new aes.ModeOfOperation.ctr(mnemonicKey, mnemonicCounter);

            var path = searchPath(data, 'x-bcbjs/path') || HDNode.defaultPath;
            ...
            signingKey = new SigningKey(node, chainId);
        }
...
export function encrypt(privateKey: Arrayish | SigningKey, chainId: string, password: Arrayish | string, options?: EncryptOptions, progressCallback?: ProgressCallback): Promise<string> {
    ...
    return new Promise(function(resolve, reject) {
        ...
        scrypt(passwordBytes, salt, N, r, p, 64, function(error, progress, key) {
            if (error) {
                ...
                var address = (new SigningKey(privateKeyBytes, chainId)).address;
                ...
                if (entropy) {
                    ...
                    data['x-bcb'] = {
                        client: client,
                        bcbFilename: ('UTC--' + timestamp + '--' + data.address),
