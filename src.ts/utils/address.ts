...
import { concat, arrayify, stripZeros, hexlify } from './bytes';
...
export function getAddressByEthAddress(chainId: string, ethAddress: Arrayish | string): string {
...
export function getEthAddressByAddress(chainId: string, address: string): string {
    if(address.indexOf(chainId) !== 0) {
        errors.throwError('chainId mismatch', errors.INVALID_ARGUMENT, { arg: 'address', value: address });
    }
    let rawB58 = address.substr(chainId.length);
