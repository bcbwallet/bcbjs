...
var JsonRpcSigner = /** @class */ (function (_super) {
    ...
    JsonRpcSigner.prototype.getAddress = function () {
        ...
        return this.provider.send('bcb_accounts', []).then(function (accounts) {
    ...
    JsonRpcSigner.prototype.sendUncheckedTransaction = function (transaction) {
        ...
        // The JSON-RPC for bcb_sendTransaction uses 90000 gas; if the user
        ...
        return Promise.all([
            ...
            return _this.provider.send('bcb_sendTransaction', [hexTx]).then(function (hash) {
                return hash;
            }, function (error) {
    ...
    JsonRpcSigner.prototype.signMessage = function (message) {
        ...
        return this.getAddress().then(function (address) {
            return _this.provider.send('bcb_sign', [address.toLowerCase(), bytes_1.hexlify(data)]);
        });
    };
...
var JsonRpcProvider = /** @class */ (function (_super) {
            ...
            var ready = new Promise(function (resolve, reject) {
                setTimeout(function () {
                    _this.send('net_version', []).then(function (result) {
                        return resolve(networks_1.getNetwork(result));
    ...
    JsonRpcProvider.prototype.listAccounts = function () {
        return this.send('bcb_accounts', []).then(function (accounts) {
    ...
    JsonRpcProvider.prototype.perform = function (method, params) {
        switch (method) {
            case 'getBlockNumber':
                return this.send('bcb_blockNumber', []);
            case 'getGasPrice':
                return this.send('bcb_gasPrice', []);
            case 'getBalance':
                return this.send('bcb_getBalance', [getLowerCase(params.address), params.blockTag]);
            case 'getTransactionCount':
                return this.send('bcb_getTransactionCount', [getLowerCase(params.address), params.blockTag]);
            case 'getCode':
                return this.send('bcb_getCode', [getLowerCase(params.address), params.blockTag]);
            case 'getStorageAt':
                return this.send('bcb_getStorageAt', [getLowerCase(params.address), params.position, params.blockTag]);
            case 'sendTransaction':
                return this.send('bcb_sendRawTransaction', [params.signedTransaction]).catch(function (error) {
                    if (error.responseText) {
            ...
            case 'getBlock':
                if (params.blockTag) {
                    return this.send('bcb_getBlockByNumber', [params.blockTag, !!params.includeTransactions]);
                }
                else if (params.blockHash) {
                    return this.send('bcb_getBlockByHash', [params.blockHash, !!params.includeTransactions]);
                }
                return Promise.reject(new Error('invalid block tag or block hash'));
            case 'getTransaction':
                return this.send('bcb_getTransactionByHash', [params.transactionHash]);
            case 'getTransactionReceipt':
                return this.send('bcb_getTransactionReceipt', [params.transactionHash]);
            case 'call':
                return this.send('bcb_call', [JsonRpcProvider.hexlifyTransaction(params.transaction, { from: true }), params.blockTag]);
            case 'estimateGas':
                return this.send('bcb_estimateGas', [JsonRpcProvider.hexlifyTransaction(params.transaction, { from: true })]);
            case 'getLogs':
                if (params.filter && params.filter.address != null) {
                    params.filter.address = getLowerCase(params.filter.address);
                }
                return this.send('bcb_getLogs', [params.filter]);
            default:
                break;
        }
    ...
    JsonRpcProvider.prototype._startPending = function () {
        ...
        var self = this;
        var pendingFilter = this.send('bcb_newPendingTransactionFilter', []);
        this._pendingFilter = pendingFilter;
        pendingFilter.then(function (filterId) {
            function poll() {
                self.send('bcb_getFilterChanges', [filterId]).then(function (hashes) {
                    ...
                    if (self._pendingFilter != pendingFilter) {
                        self.send('bcb_uninstallFilter', [filterId]);
                        return;
                    }
    ...
    // Convert an bcbjs.js transaction into a JSON-RPC transaction
    //  - gasLimit => gas
