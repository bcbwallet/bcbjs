...
function encodeSignature(fragment) {
    var frag = parseSignature(fragment);
    var sig = formatSignature(frag);
    var hash = js_sha3_1.keccak_256.update(sig).digest();
    if (frag.type === 'event') {
        return bytes_1.hexlify(hash);
    }
    else {
...
function getParamCoder(coerceFunc, param) {
    ...
    if (param.type === 'byte') {
        return new CoderFixedBytes(coerceFunc, 1, param.name);
    }
