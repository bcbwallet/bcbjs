...
function encryptSecret(plain, password) {
    ...
    //     plainBytes = bytes;
    // } else {
    plainBytes = bytes_1.arrayify(plain);
    // }
    var pass = pbkdf2_1.pbkdf2(password, 'e91e2fb05854a2a0a093be59b45944df73e3d0dbfd46e90935f527e324669330', 1, 32, 'sha256');
    var key = pass.slice(0, 16);
    var iv = pass.slice(16);
    // Encrypt
    plainBytes = aes_js_1.default.padding.pkcs7.pad(plainBytes);
    var aesCbc = new aes_js_1.default.ModeOfOperation.cbc(key, iv);
    var cipher = aesCbc.encrypt(plainBytes);
    return cipher;
}
exports.encryptSecret = encryptSecret;
function decryptSecret(cipher, password) {
    password = getPassword(password);
    ...
    try {
        plain = aes_js_1.default.padding.pkcs7.strip(plain);
    }
    catch (e) {
...
function decrypt(json, password, progressCallback) {
    ...
    var getSigningKey = function (key, reject) {
        ...
        var signingKey = new signing_key_1.SigningKey(privateKey, chainId);
        ...
        if (searchPath(data, 'x-bcbjs/version') === '0.1') {
            ...
            signingKey = new signing_key_1.SigningKey(node, chainId);
        }
        return signingKey;
    };
...
function encrypt(privateKey, chainId, password, options, progressCallback) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        scrypt_js_1.default(passwordBytes, salt, N, r, p, 64, function (error, progress, key) {
            if (error) {
                ...
                var address = (new signing_key_1.SigningKey(privateKeyBytes, chainId)).address;
