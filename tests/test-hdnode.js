...
describe('Test HD Node Derivation', function(test) {
    ...
    tests.forEach(function(test) {
        ...
        it('Derives the HD nodes - ' + test.name, function() {
            ...
            //var rootNode = new bcbjs.utils.HDNode.fromSeed(test.seed);
            var rootNode = new bcbjs.utils.HDNode.fromMnemonic(test.mnemonic, null, test.password || null);
            ...
            test.hdnodes.forEach(function(nodeTest) {
                ...
                var wallet = new bcbjs.Wallet(node.privateKey);
                assert.equal(wallet.address.toLowerCase(), nodeTest.address,
                    'Generates address - ' + nodeTest.privateKey);

                assert.equal(node.address, (new bcbjs.Wallet(node)).address,
                ...
                var hardNode = rootNode.derivePath(lastHardened[1] + "'");
                    ...
                    var nodeXpriv = bcbjs.utils.HDNode.fromExtendedKey(hardNode.extendedKey);
                    nodeXpriv = nodeXpriv.derivePath(lastHardened[2].substring(1));
                    var nodeXpub = bcbjs.utils.HDNode.fromExtendedKey(neutered.extendedKey);
                ...
                // Test serialization
                var deserializedNode = bcbjs.utils.HDNode.fromExtendedKey(nodeTest.xpriv);
...
describe('Test HD Mnemonic Phrases', function testMnemonic() {
    ...
    tests.forEach(function(test) {
        ...
        it(('converts mnemonic phrases - ' + test.name), function() {
            ...
            assert.equal(bcbjs.utils.HDNode.entropyToMnemonic(test.entropy), test.mnemonic,
                'Converts entropy to mnemonic ' + test.name);
            assert.equal(bcbjs.utils.HDNode.mnemonicToEntropy(test.mnemonic), test.entropy,
                'Converts mnemonic to entropy - ' + test.mnemonic);
            assert.equal(bcbjs.utils.HDNode.mnemonicToSeed(test.mnemonic, test.password), test.seed,
    ...
    describe('Test easyseed BIP39 Test cases - ' + locale, function() {
        ...
        tests.forEach(function(test) {
            it('test - ' + test.entropy, function() {
                this.timeout(100000);
                var seed = bcbjs.utils.HDNode.mnemonicToSeed(test.mnemonic, test.passphrase);
                assert.equal(seed, '0x' + test.seed, 'seeds match');

                var entropy = bcbjs.utils.HDNode.mnemonicToEntropy(test.mnemonic, lang);
                assert.equal(entropy, '0x' + test.entropy, 'entropy match');

                var mnemonic = bcbjs.utils.HDNode.entropyToMnemonic('0x' + test.entropy, lang);
...
testEasySeed(bcbjs.wordlists.en, 'en');
testEasySeed(bcbjs.wordlists.es, 'es');
testEasySeed(bcbjs.wordlists.fr, 'fr');
testEasySeed(bcbjs.wordlists.ja, 'ja');
testEasySeed(bcbjs.wordlists.zh_cn, 'zh_cn');
testEasySeed(bcbjs.wordlists.zh_tw, 'zh_tw');
testEasySeed(bcbjs.wordlists.it, 'it');
testEasySeed(bcbjs.wordlists.ko, 'ko');
...
describe('Testnet Extended Key (#553)', function testMnemonic() {
    ...
    tests.forEach(function(test) {
        it(test.name, function() {
            var node = bcbjs.utils.HDNode.fromExtendedKey(test.extended);
...
describe("Test Mnemonic is Case Agnostic", function() {
    ...
    function addTest(mnemonic, altMnemonic) {
        it(altMnemonic, function() {
            var node = bcbjs.utils.HDNode.fromMnemonic(mnemonic);
            var altNode = bcbjs.utils.HDNode.fromMnemonic(altMnemonic);
    ...
    for (var i = 0; i < 128; i++) {
        ...
        var mnemonic = bcbjs.utils.HDNode.entropyToMnemonic(entropy);
