...
describe('Test Contract Address Generation', function() {
    ...
    var getContractAddress = bcbjs.utils.getContractAddress;
...
describe('Test Unit Conversion', function () {
    ...
    tests.forEach(function(test) {
        var wei = bcbjs.utils.bigNumberify(test.wei);
        ...
        it (('parses ' + test.ether + ' ether'), function() {
            assert.ok(bcbjs.utils.parseEther(test.ether.replace(/,/g, '')).eq(wei),
                'parsing ether failed - ' + test.name);
        });

        it (('formats ' + wei.toString() + ' wei (options: ' + JSON.stringify(formatting) + ')'), function() {
            var v = bcbjs.utils.formatEther(wei);
            if (formatting.commify) { v = bcbjs.utils.commify(v); }
        ...
        ['kwei', 'mwei', 'gwei', 'szabo', 'finney', 'satoshi'].forEach(function(name) {
            ...
            if (test[name]) {
                it(('parses ' + test[name] + ' ' + name), function() {
                    this.timeout(120000);
                    assert.ok(bcbjs.utils.parseUnits(test[name].replace(/,/g, ''), unitName).eq(wei),
            ...
            if (test[name + '_format']) {
                it (('formats ' + wei.toString() + ' ' + name + ' (options: ' + JSON.stringify(formatting) + ')'), function() {
                    var v = bcbjs.utils.formatUnits(wei, unitName);
                    if (formatting.commify) { v = bcbjs.utils.commify(v); }
...
describe('Test Namehash', function() {
    ...
    tests.forEach(function(test) {
        it(('computes namehash - "' + test.name + '"'), function() {
            this.timeout(120000);
            assert.equal(bcbjs.utils.namehash(test.name), test.expected,
...
describe('Test ID Hash Function', function () {
    ...
    tests.forEach(function(test) {
        it(('computes id - ' + test.name), function() {
            this.timeout(120000);
            var value = bcbjs.utils.id(test.text);
    ...
    ['Keccak256', 'Sha256'].forEach(function(funcName) {
        it(('computes ' + funcName + ' correctly'), function() {
            ...
            tests.forEach(function(test, index) {
                var result = bcbjs.utils['solidity' + funcName](test.types, test.values);
...
describe('Test Hash Functions', function() {
    ...
    it('computes keccak256 correctly', function() {
        this.timeout(120000);
        tests.forEach(function(test) {
            assert.equal(bcbjs.utils.keccak256(test.data), test.keccak256, ('Keccak256 - ' + test.data));
        });
    });
    ...
    it('computes sha2566 correctly', function() {
        this.timeout(120000);
        tests.forEach(function(test) {
            assert.equal(bcbjs.utils.sha256(test.data), test.sha256, ('SHA256 - ' + test.data));
        });
    });
});
...
describe('Test Solidity splitSignature', function() {
    ...
    it('splits a canonical signature', function() {
        ...
        for (var v = 27; v <= 28; v++) {
            var signature = bcbjs.utils.concat([ r, s, [ v ] ]);
            var sig = bcbjs.utils.splitSignature(signature);
    ...
    it('splits a legacy signature', function() {
        ...
        var s = '0xcafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7e';
        for (var v = 27; v <= 28; v++) {
            var signature = bcbjs.utils.concat([ r, s, [ v - 27 ] ]);
            var sig = bcbjs.utils.splitSignature(signature);
            assert.equal(sig.r, r, 'split r correctly');
            assert.equal(sig.s, s, 'split s correctly');
            assert.equal(sig.v, v, 'split v correctly');
...
describe('Test Base64 coder', function() {
    ...
    it('encodes and decodes the example from wikipedia', function() {
        ...
        var decoded = bcbjs.utils.toUtf8Bytes(decodedText);
        ...
        assert.equal(bcbjs.utils.base64.encode(decoded), encoded, 'encodes to base64 string');
        assert.equal(bcbjs.utils.toUtf8String(bcbjs.utils.base64.decode(encoded)), decodedText, 'decodes from base64 sstring');
    });
});
...
describe('Test UTF-8 coder', function() {
    ...
    BadUTF.forEach(function(test) {
        it('toUtf8String - ' + test.name, function() {
            assert.throws(function() {
                var result = bcbjs.utils.toUtf8String(test.bytes);
    ...
    it('toUtf8String - random conversions', function() {
        ...
        for (var i = 0; i < 100000; i++) {
            ...
            var bytes = bcbjs.utils.toUtf8Bytes(str)
            var str2 = bcbjs.utils.toUtf8String(bytes);
...
describe('Test Bytes32String coder', function() {
    ...
    it("encodes an ens name", function() {
        ...
        var bytes32 = bcbjs.utils.formatBytes32String(str);
        var str2 = bcbjs.utils.parseBytes32String(bytes32);
...
describe('Test BigNumber', function() {
    it("computes absoltue values", function() {
        function testAbs(test) {
            var value = bcbjs.utils.bigNumberify(test.value);
            var expected = bcbjs.utils.bigNumberify(test.expected);
...
describe("Hexlify", function() {
    it("hexlify on string of unsafe number", function() {
        assert(bcbjs.utils.hexlify(bcbjs.utils.bigNumberify("9985956830000000000")), "0x8a953ed43a892c00", "hexlify on large BigNumber");
    });

    [9007199254740991, 9985956830000000000].forEach(function(value) {
        it('hexlify fails on unsafe number - ' + value, function() {
            assert.throws(function() {
                var result = bcbjs.utils.hexlify(value);
                console.log('Result', result);
            }, function(error) {
