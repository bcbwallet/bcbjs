...
//var providers = bcbjs.providers;
var bigNumberify = bcbjs.utils.bigNumberify;
var getAddress = bcbjs.utils.getAddress;
    ...
    describe(('Read-Only ' + providerName + ' (' + networkName + ')'), function() {
        ...
        if (networkName === 'default') {
            if (providerName === 'getDefaultProvider') {
                provider = bcbjs.getDefaultProvider();
            } else if (providerName === 'Web3Provider') {
                var infuraUrl = (new bcbjs.providers.InfuraProvider()).connection.url;
                provider = new bcbjs.providers.Web3Provider(new Web3HttpProvider(infuraUrl));
            } else {
                provider = new bcbjs.providers[providerName]();
            }
        } else {
            if (providerName === 'getDefaultProvider') {
                provider = bcbjs.getDefaultProvider(networkName);
            } else if (providerName === 'Web3Provider') {
                var infuraUrl = (new bcbjs.providers.InfuraProvider(networkName)).connection.url;
                provider = new bcbjs.providers.Web3Provider(new Web3HttpProvider(infuraUrl), networkName);
            } else {
                provider = new bcbjs.providers[providerName](networkName);
            }
        }
...
function getDefaults(network, extra) {
    var network = bcbjs.utils.getNetwork(network);
    var result = {
...
describe('Test Basic Authentication', function() {
    ...
    //var Provider = bcbjs.Provider;

    function test(name, url) {
        it('tests ' + name, function() {
            this.timeout(20000);
            return bcbjs.utils.fetchJson(url).then(function(data) {
    ...
    it('tests insecure connections fail', function() {
        ...
        assert.throws(function() {
            bcbjs.utils.fetchJson(insecure);
        }, function(error) {
