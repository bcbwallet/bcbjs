...
describe('Test JSON Wallets', function() {
    var Wallet = bcbjs.Wallet;
    var tests = utils.loadTests('wallets');
    tests.forEach(function(test) {
        it(('decrypts wallet - ' + test.name), function() {
            this.timeout(1200000);

            assert.ok((bcbjs.utils.getJsonWalletAddress(test.json) !== null),
...
function checkTransaction(parsedTransaction, test) {
    ...
    ['nonce', 'gasLimit', 'gasPrice', 'to', 'value', 'data'].forEach(function(key) {
        ...
        if ({ gasLimit: 1, gasPrice: 1, value: 1 }[key]) {
            assert.ok((bcbjs.utils.BigNumber.isBigNumber(value)),
...
describe('Test Transaction Signing and Parsing', function() {
    ...
    tests.forEach(function(test) {
        it(('parses and signs transaction - ' + test.name), function() {
            ...
            var signingKey = new bcbjs.utils.SigningKey(test.privateKey);
            var signDigest = signingKey.signDigest.bind(signingKey);

            // Legacy parsing unsigned transaction
            checkTransaction(bcbjs.utils.parseTransaction(test.unsignedTransaction), test);

            var parsedTransaction = bcbjs.utils.parseTransaction(test.signedTransaction);
            var transaction = checkTransaction(parsedTransaction, test);

            // Legacy signed transaction ecrecover
            assert.equal(parsedTransaction.from, bcbjs.utils.getAddress(test.accountAddress),
                'computed from');
            ...
            (function() {
                var unsignedTx = bcbjs.utils.serializeTransaction(transaction);
                ...
                var signature = signDigest(bcbjs.utils.keccak256(unsignedTx));
                assert.equal(bcbjs.utils.serializeTransaction(transaction, signature), test.signedTransaction,
            ...
            var parsedUnsignedTransactionChainId5 = bcbjs.utils.parseTransaction(test.unsignedTransactionChainId5);
            ...
            // EIP-155 fields
            var parsedTransactionChainId5 = bcbjs.utils.parseTransaction(test.signedTransactionChainId5);
            ...
            (function() {
                ...
                var signature = signDigest(bcbjs.utils.keccak256(unsignedTx));
                assert.equal(bcbjs.utils.serializeTransaction(transaction, signature), test.signedTransactionChainId5,
...
describe('Test Signing Messages', function() {
    ...
    var arrayify = bcbjs.utils.arrayify;
    var id = bcbjs.utils.id;
    var toUtf8Bytes = bcbjs.utils.toUtf8Bytes;

    var tests = [
        // See: https://ethscan.io/verifySig/57
        {
        ...
        // See: https://github.com/bcbjs-io/bcbjs.js/issues/80
        {
        ...
        // See: https://github.com/bcbjs-io/bcbjs.js/issues/85
        {
    ...
    tests.forEach(function(test) {
        it(('verifies a message "' + test.name + '"'), function() {
            this.timeout(120000);
            var address = bcbjs.utils.verifyMessage(test.message, test.signature);
    ...
    tests.forEach(function(test) {
      it(('hashes a message "' + test.name + '"'), function() {
          this.timeout(120000);
          var hash = bcbjs.utils.hashMessage(test.message);
