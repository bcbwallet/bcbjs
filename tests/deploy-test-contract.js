...
var provider = bcbjs.providers.getDefaultProvider('rinkeby');
var wallet = new bcbjs.Wallet(fs.readFileSync('.account.key').toString().trim(), provider);
...
var deployTransaction = bcbjs.Contract.getDeployTransaction(result.bytecode, result.interface);
...
wallet.sendTransaction(deployTransaction).then(function(tx) {
    ...
    result.contractAddress = bcbjs.utils.getContractAddress(tx);
