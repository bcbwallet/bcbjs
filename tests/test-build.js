...
describe("Package Version", function() {
    var url = "http://registry.npmjs.org/bcbjs"
    it("is not already published", function() {
        this.timeout(20000);
        return bcbjs.utils.fetchJson(url).then(function(data) {
            assert.ok(Object.keys(data.versions).indexOf(bcbjs.version) === -1);
        });
    });
});

// These test cases will fail if "npm run dist" has not been called to
// create the ./dist/bcbjs.js and ./dist/bcbjs.min.js. They will also
...
describe("Dist Build:", function() {
    ...
    function test(filename) {
        it("matches dist build - " + filename, function() {
            ...
            assert.equal(bcbjs.utils.id(checkDist), bcbjs.utils.id(dist), 'matches');
        });
    }
    ...
    it("does not have host specific includes", function() {
        var data = fs.readFileSync(path.resolve(__dirname, '../dist/bcbjs.types.txt')).toString();
