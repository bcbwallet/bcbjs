...
function equals(actual, expected) {
    ...
    // Maybe address?
    try {
        var actualAddress = bcbjs.utils.getAddress(actual);
        var expectedAddress = bcbjs.utils.getAddress(expected);
...
describe('ABI Coder Encoding', function() {
    var coder = bcbjs.utils.defaultAbiCoder;
...
describe('Test Contract Events', function() {
    ...
    tests.forEach(function(test, index) {
        it(('decodes event parameters - ' + test.name + ' - ' + test.types), function() {
            ...
            var contract = new bcbjs.utils.Interface(test.interface);
        ...
        it(('decodes event data - ' + test.name + ' - ' + test.types), function() {
            ...
            test.normalizedValues.forEach(function(expected, index) {
                if (test.indexed[index]) {
                    assert.ok((bcbjs.Contract.isIndexed(parsed[index]) && parsed[index].hash == null), 'parsed event data has empty Indexed - ' + index);
                } else {
...
describe('Test Interface Signatures', function() {
    var Interface = bcbjs.utils.Interface;
    ...
    // See: https://github.com/bcbjs-io/bcbjs.js/issues/370
    it ('parses transaction function', function() {
        ...
        var tx = bcbjs.utils.parseTransaction(rawTx);
...
describe('Test Number Coder', function() {
    var coder = bcbjs.utils.defaultAbiCoder;
    var bnify = bcbjs.utils.bigNumberify;
    ...
    it('encodes zero', function() {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: bcbjs.constants.Zero }
        ];
    ...
    it('encodes one', function() {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: bcbjs.constants.One }
        ];
    ...
    it('encodes negative one', function() {
        var Values = [
            ...
            { n: 'hex leading even length', v: '-0x0001' },
            { n: 'hex leading odd length', v: '-0x00001' },
            { n: 'BigNumber', v: bcbjs.constants.NegativeOne }
        ];
    ...
    it('encodes full uint8 range', function() {
        for (var i = 0; i < 256; i++) {
            ...
            expected += bcbjs.utils.hexlify(i).substring(2);
    ...
    it('encodes full int8 range', function() {
        for (var i = -128; i < 128; i++) {
            ...
            if (i === -128) {
                ...
                expected += bcbjs.utils.hexlify(256 + i).substring(2);
            } else {
...
describe('Test Filters', function() {
    ...
    function doTest(test) {
        it(test.name, function() {
            var iface = new bcbjs.utils.Interface([ test.signature ]);
    ...
    var Tests = [
        ...
        // See: https://github.com/bcbjs-io/bcbjs.js/issues/305
        {
        ...
        // https://ethscan.io/tx/0x820cc57bc77be44d8f4f024a18e18f64a8b6e62a82a3d7897db5970dbe181ba1
        {
