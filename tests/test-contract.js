...
var provider = new bcbjs.providers.InfuraProvider('rinkeby');

var contract = (function() {
    var data = require('./test-contract.json');
    return new bcbjs.Contract(data.contractAddress, data.interface, provider);
})();
...
function equals(name, actual, expected) {
    ...
    if (typeof(actual) === 'object') {
        if (expected.indexed) {
            assert.ok(bcbjs.Contract.isIndexed(actual), 'index property has index - ' + name);
            if (expected.hash) {
...
function TestContractEvents() {
    return bcbjs.utils.fetchJson('https://api.bcbjs.io/api/v1/?action=triggerTest&address=' + contract.address).then(function(data) {
